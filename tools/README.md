...
| `translate_tag_zh.py` | 给中文对照跑 AI 长尾补译，只翻「人工表和词库都没有」的键，断点续跑；渠道配置在 Git 忽略的 `tools/data/tag_zh/ai_config.json`（含密钥，不写日志、不写报告）；同时命中未成年指示词与性相关词的键有安全闸，不送外部接口。跑完必须再跑 `build_tag_zh.py` 才会进分片 | **会调用外部 AI 接口并产生费用**；追加写 `tools/data/tag_zh/AI译名.csv`，覆盖写 `output/tag-zh/` 的跑批日志 / 报告 / 失败清单；`--dry-run` 只统计 |
