...
搜索框默认只匹配标题、标签和角色正向提示词；空格表示“同时满足”，双引号表示完整短语，`-词` 表示排除。需要按负面提示词、备注、作者、法典、类型、路径、图片或收藏状态查找时，点击“筛选”添加中文条件。目录文字命中会单独列在“相关目录”，不会再把整个目录的图片混入结果。
...
新增分类或合并书目等依赖新程序的数据，应先走菜单 5。程序部署完成后运行菜单 4，首次确认兼容文件已上线时会记录浏览器旧缓存的等待窗口，并显示可再次发布的时间；等待期间不上传数据，届时重新运行菜单 4 即可。普通数据更新会复用已经通过的兼容记录，但每次仍核对线上文件。
