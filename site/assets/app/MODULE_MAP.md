...
| --- | --- | --- | --- | --- |
| `../app.js` | 主站组合根；`init`、`loadCodex`、收藏视图、全站搜索视图、筛选与搜索编排 | 法典加载序号、收藏备份绑定标记 | `state.js`、`utils.js`、`feedback.js`、`access.js`、`data.js`、`search.js`、`media.js`、`favorites.js`、`favorites-backup-core.js`、`favorites-backup.js`、`fav-codex.js`、`site-search.js`、`masonry.js`、`lightbox.js`、`copy.js`、`report.js`、`router.js`、`codex-route-compat.js`、`path-code.js`、`codex-ui.js`、`history.js`、`ui.js`、`onboarding.js`、`intro.js`、`resume-prompt.js`、`browser-history.js`、`tag-relay.js` | 先完成全部 `set*Actions(...)`，再调用 `init()`；本地编辑器满足探测条件后才动态导入 `edit.js` |
...
| `state.js` | 全局运行态、存储键、密度和搜索范围规范化 | 共享 `state` 对象 | — | 只放跨模块运行态；模块私有状态留在所属模块 |
...
| `browser-history.js` | 页面无关的路由记录、覆盖层栈、恢复令牌与滚动检查点 | 当前记录、恢复状态、待返回操作、层注册表与计时器 | — | 页面通过 `configureBrowserHistory` 注入 `captureRoute`、`urlForRoute`、`applyRoute`、`restoreScroll`、`isEmptySearchRoute` |
...
| `search.js` | 查询解析、短语匹配、高亮、隐藏命中判断与缓存失效 | 每词条可搜索文本 `WeakMap` 缓存 | `state.js`、`utils.js`、`media.js`、`favorites.js` | 编辑器原地改词条后必须调用 `invalidateSearchableText` |
| `router.js` | URL 读写、规范标题、分享路径、历史适配与词条深链 | `routerActions` | `state.js`、`utils.js`、`path-code.js`、`media.js`、`feedback.js`、`access.js`、`browser-history.js` | 注入 `onUrlSync`、`renderTree`、`applyFilter`、`openLightbox`、`updateVirtualCards`、`applyHistoryRoute`、`restoreHistoryScroll`；规范地址规则见本地私有文档 `docs/decisions/短链与地址栏规范地址.md` |
...
| `fav-codex.js` | 构建“全部收藏”虚拟法典 | — | `state.js`、`data.js`、`access.js`、`media.js`、`favorites-backup-core.js` | 不写入 `state.codexes`；给条目补 `_src*`，使收藏、历史、反馈与媒体仍指向真实法典 |
| `site-search.js` | 构建并失效“全站搜索”虚拟法典 | 成功结果缓存、在途 Promise、失效代次 | `state.js`、`data.js`、`access.js`、`media.js` | 只缓存完整成功且非降级的结果；编辑后显式失效；条目同样携带 `_src*` |
...
| `announcements.js` | 公告加载、面板和未读角标 | 公告数据、加载状态与在途 Promise | `utils.js`、`modal.js`、`history.js`、`../data-source.js` | 数据读取走统一数据源，不自行拼发布路径 |
...
| `ui.js` | 全局事件绑定、主题 / 密度 / 搜索范围 / 分级开关与页面级编排 | `uiActions`、主题图标表 | `state.js`、`utils.js`、`feedback.js`、`access.js`、`codex-ui.js`、`router.js`、`history.js`、`masonry.js`、`lightbox.js`、`clipboard-fallback.js`、`modal.js`、`announcements.js`、`report.js`、`onboarding.js`、`tag-relay-rail.js`、`tag-relay.js`、`home-shortcut.js`、`resume-prompt.js`、`browser-history.js` | 注入 `loadCodex`、两个虚拟视图入口、`exitSiteSearchView`、`applySearch`、`applyFilter`；`applyDensity` 仍是跨状态、DOM 与布局的编排点 |
...
| 模块 | 职责 / 主要出口 | 模块状态 | 直接依赖 | 注入 / 边界 |
| --- | --- | --- | --- | --- |
| `edit.js` | `initEditMode`；本地词条、目录、法典与图片编辑 UI | 服务端能力、注入动作、启用 / 保存 / 弹层与当前词条状态 | `state.js`、`utils.js`、`feedback.js`、`lightbox.js`、`codex-ui.js`、`modal.js`、`../data-source.js`、`search.js`、`masonry.js`、`site-search.js`、`edit-core.js` | `app.js` 探测同源 `/__edit__/ping` 成功后才动态导入；接收 `loadCodex`、`applyFilter`、`captureRoute`，并向 `codex-ui.js` 追加 `decorateDoor`；线上不得静态加载 |
