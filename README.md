...
- ✏️ **本地编辑模式**：直接增删词条、调整分类和图片，也能从空数据创建自己的法典

## 🚀 本地使用（Windows 双击即用）
> 前置：本机装好 Python 3，并 `pip install -r requirements.txt`

**大多数日常操作可从 `法典图鉴.bat` 总控台启动；本地编辑器目前直接双击 `单项工具\法典编辑器.bat`。**

1. **手工调整法典**：双击 `单项工具\法典编辑器.bat` → 打开 http://localhost:8769 → 点击顶栏铅笔按钮
2. **从 Word 加法典**：把法典 `.docx` 放进 `法典源/` → 总控台选 `5`（转换法典）
3. **批量配图**：总控台选 `2`（配图工具）→ 把图拖到对应词条上（自动压缩、命名、写入本地缓存）
4. **只读预览**：总控台选 `1`（启动预览）→ 打开 http://localhost:8766
5. **同步图片**：复制 `r2_config.example.json` 为 `r2_config.json`，填入 Cloudflare R2 信息 → 总控台选 `3`（同步 R2）
6. **发布**：总控台选 `4`（发布）（先同步 R2，再 git push → Cloudflare 自动部署）

转换器还会生成 `site/data/待复核_*.txt`，列出极少数可能解析有误的词条，供人工复核。

### 本地编辑模式

- 只监听 `127.0.0.1`，普通线上站点不会加载编辑代码。
- 支持词条增删改与重分类、单图增删、目录增删改移，以及法典新建、删除和元信息维护。
- 每次保存前都会备份到 `output/edit-backups/`；图片变更后，发布前仍需运行“同步 R2”。
- 不要同时启动法典编辑器和配图工具，避免两个本地写工具同时修改同一本数据。
- `法典源/` 中仍有对应 docx 时，页面会显示覆盖风险警告；手工调整后不要直接重跑转换器。
